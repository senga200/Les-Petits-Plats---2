...
function searchRecipes() {
  ...
  if (input.length > 2) {
    recipesTab.filter((recipe) => {
      const name = recipe.name.toLowerCase();
      const appliance = recipe.appliance.toLowerCase();

      if (name.includes(input) || appliance.includes(input)) {
        results.push(recipe);
      } else {
        ...
        recipe.ingredients.forEach((ingredient) => {
          //on parcourt le tableau d'ingrédients
        ...
        if (foundIngredient || foundUstensil) {
          results.push(recipe);
        }
      }
    });
...
function beAnUstensilTag(e) {
  if (e.target.tagName === "LI") {
    //si l'element cliqué est un li
    const ustensil = e.target.textContent.trim().toLowerCase(); //recupere le texte du li
...
function beAnApplianceTag(e) {
  if (e.target.tagName === "LI") {
    //si l'element cliqué est un li
    const appliance = e.target.textContent.trim().toLowerCase(); //recupere le texte du li
