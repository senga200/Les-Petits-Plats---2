...
async function getRecipe() { //results en parametre? 
  try {
...
function displayResults(results){
  ...
  results.forEach(recipe => {//pour chaque recette du tableau results
    const recipeDetail = recipeFactory(recipe);//on appelle la factory que l'on stocke dans la variable recipeDetail
    const recipeElement = recipeDetail.getRecipeDOM();//on applique getRecipeDOM à recipeDetail
    grid.appendChild(recipeElement);//on ajoute le resultat de getRecipeDOM à grid
  }); 
  //getIngredients(results);
  //getAppliances(results);
  //getUstensils(results);
...
searchInput.addEventListener("input", function() {
  ...
  if (results.length > 0) {//si le tableau des resultats contient des recettes
    results = [...new Set(results)];//recupère la liste des résultats de la recherche =" results" supprime les doublons et ... permet de transformer le set en tableau pour pouvoir l'utiliser dans la fonction displayResults, besoin, pas besoin ?
